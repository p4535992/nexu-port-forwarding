...
public final class MobaXtermImport {
    ...
    private static Candidate parseEntry(String key,String value,String label,int lineNo) {
        ...
        String proxy=f.length>7?f[7].trim():"";
        if(!proxy.isEmpty()&&!proxy.equalsIgnoreCase("No proxy selected"))
            throw new Invalid("proxy MobaXterm configurato: importazione rifiutata per evitare una connessione differente.");
        ...
        TunnelProfile base=new TunnelProfile(UUID.randomUUID(),display,mode,ssh.host,ssh.port,ssh.user,bind,listenPort,
            ...
            "Importato da MobaXterm. Password, proxy, trust host e dati esterni non importati.",install,TunnelProfile.Origin.MOBAXTERM,"");
