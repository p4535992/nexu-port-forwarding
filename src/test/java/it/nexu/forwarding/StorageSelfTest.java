        ...
        try {
            ...
            try(AppLog log=new AppLog(home)) {
                ...
                log.event(new TunnelEngine.Event(profile.id(),TunnelEngine.State.ERROR,
                    ...
                    Instant.now()));
                log.event(new TunnelEngine.Event(profile.id(),TunnelEngine.State.ERROR,
                    "Errore runtime SSH: dipendenza Java mancante (org.example.MissingRuntimeClass). Il pacchetto dell'applicazione è incompleto o incompatibile.",
                    Instant.now()));
            }
            ...
            check(logText.contains("category=RUNTIME_DEPENDENCY_MISSING")&&logText.contains("org.example.MissingRuntimeClass"),"persistent log records packaged runtime failures");
